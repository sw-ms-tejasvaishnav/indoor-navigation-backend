/*****************************************************************************/
/* Source Code Modified to better serve needs of MaaS Portal. DO NOT UPDATE! */
/*****************************************************************************/
...
L.drawLocal = {
    draw: {
        toolbar: {
            ...
            buttons: {
                point: 'Create a waypoint',
                marker: 'Create a point of interest',
                inaccessibleWayPoint: 'Create an inaccessible waypoint',
                polyline: 'Connect route segments',
                polygon: 'Draw a polygon',
                rectangle: 'Draw a rectangle',
                circle: 'Create a zone'
            }
        },
        handlers: {
            circle: {
                tooltip: {
                    start: 'Click and drag to create a zone.'
                }
            },
            point: {
                tooltip: {
                    start: 'Click map to place a waypoint for a route segment.'
                }
            },
            marker: {
                tooltip: {
                    start: 'Click map to place a point of interest.'
                }
            },
            inaccessibleWayPoint: {
                tooltip: {
                    start: 'Click map to place an inaccessible waypoint.'
                }
            },
            polygon: {
                tooltip: {
            ...
            polyline: {
                ...
                tooltip: {
                    start: 'Step 1: Click on a waypoint or POI to start drawing a segment.',
                    cont: 'Step 2: Click on another waypoint or POI to finish drawing a segment.',
                    end: 'Step 2: Click on another waypoint or POI to finish drawing a segment.'
                }
            },
            rectangle: {
                tooltip: {
    ...
    edit: {
        ...
        handlers: {
            edit: {
                tooltip: {
                    text: 'Drag handles and markers to edit feature size and placement.',
...
//        this._updateTooltip();
     ...
     //console.log('pts', manager.segmentPts);
...
L.Draw.Point = L.Draw.Feature.extend({
    statics: {
        TYPE: 'point'
    },
    ...
    addHooks: function () {
        ...
        if (this._map) {
            this._tooltip.updateContent({ text: L.drawLocal.draw.handlers.point.tooltip.start });
...
L.Draw.InAccessibleWaypoint = L.Draw.Feature.extend({
    statics: {
        TYPE: 'inaccessibleWayPoint'
    },
    ...
    initialize: function (map, options) {
        // Save the type so super can fire, need to do this as cannot do this.TYPE :(
        this.type = L.Draw.InAccessibleWaypoint.TYPE;
    ...
    addHooks: function () {
        ...
        if (this._map) {
            this._tooltip.updateContent({ text: L.drawLocal.draw.handlers.inaccessibleWayPoint.tooltip.start });
...
L.Toolbar = L.Class.extend({
    ...
    _initModeHandler: function (handler, container, buttonIndex, classNamePredix, buttonTitle, permission) {
    ...
    _createButton: function (options) {
        ...
        if (options.permission === true) {
            ...
            link.className = options.className +' not-allowed';
        }
    ...
    _createActions: function (buttons) {
        ...
        for (var i = 0; i < l; i++) {
            ...
            button = this._createButton({
                ...
                permission:  buttons[i].permission
            });
    ...
    addToolbar: function (map) {
        ...
        if (this.options.marker) {
            this._initModeHandler(
                ...
                this.options.marker.icon.options.permission
            );
        }

        if (this.options.point) {
            this._initModeHandler(
                new L.Draw.Point(map, this.options.point),
                this._toolbarContainer,
                buttonIndex++,
                buttonClassPrefix,
                L.drawLocal.draw.toolbar.buttons.point,
                this.options.point.icon.options.permission
            );
        }

        if (this.options.inaccessibleWayPoint) {
            this._initModeHandler(
                new L.Draw.InAccessibleWaypoint(map, this.options.inaccessibleWayPoint),
                this._toolbarContainer,
                buttonIndex++,
                buttonClassPrefix,
                L.drawLocal.draw.toolbar.buttons.inaccessibleWayPoint,
                this.options.inaccessibleWayPoint.icon.options.permission
            );
        }
        ...
        if (this.options.polyline) {
            this._initModeHandler(
                ...
                this.options.polyline.shapeOptions.permission
            );
        }
        ...
        if (this.options.circle) {
            this._initModeHandler(
                ...
                this.options.circle.shapeOptions.permission
            );
        }
